...
> EDINETの有価証券報告書XBRLが「どんなファイルで・どんな用語でできているか」の入門資料。実例はすべて **ＡＲＥホールディングス（有価証券報告書 第17期 / docID S100YBFP）** から。
...
有価証券報告書のダウンロード種別（EDINET API v2 の `type`）：
...
| `jpcrp_cor`       | 企業内容開示（有価証券報告書の様式・経営指標等）         |
...
→ この「人手だと辛い構造」を、Claude Code に解かせてまとめて可視化するのが今回の主眼。

### 会計基準の違い：本来は大変 → Claude Codeなら簡単

![会計基準の自動判定と分岐](../会計基準分岐.svg)

有価証券報告書には **IFRS と 日本基準（Japan GAAP）** が混在する。手で実装するなら、`AccountingStandardsDEI` で基準を判定し、基準ごとに拾う要素を分岐し（IFRS `RevenueIFRS…` ／ 日本基準 `NetSales…`）、さらに銀行など様式の違う業種まで面倒を見る必要がある——ここが本来いちばん手間のかかる部分。Claude Code はこの「調べて・分岐を書いて・スキルにまとめる」までを自走し、**難しさをエージェントが一度だけ担い、スキルの中に収める**。結果、使う側から見れば「どの会社を指しても同じ一言で動く」体験になる。これが"本来は大変→簡単"の中身であり、**違いが消えるのではなく、複雑さの担い手が人からエージェントへ移る**点がポイント。
