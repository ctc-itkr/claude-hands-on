...
### 会計基準の違い：本来は大変 → Claude Codeなら簡単（訴求の核）

有報には **IFRS と 日本基準（Japan GAAP）** が混在する。手で実装するなら、`AccountingStandardsDEI` で基準を判定し、基準ごとに拾う要素を分岐し（IFRS `RevenueIFRS…` ／ 日本基準 `NetSales…`）、さらに銀行など様式の違う業種まで面倒を見る必要がある——ここが本来いちばん手間のかかる部分。Claude Code はこの「調べて・分岐を書いて・スキルに畳み込む」までを自走し、**難しさをエージェントが一度だけ引き受けてスキルに隠蔽**する。結果、参加者から見れば「どの会社を指しても同じ一言で動く」体験になる。これが"本来は大変→簡単"の中身であり、**違いが消えるのではなく、複雑さの担い手が人からエージェントへ移る**点がポイント。
...
> 日本基準6社＝三菱UFJ・三井住友・みずほ（銀行）／キーエンス／東京エレクトロン／松田産業、残り11社はIFRS。銀行は「売上高＝経常収益」「自己資本比率＝BIS規制比率ではない」等、業種特有の読み替えが要る点も一言添えると誤読を防げる。スクリプトは基準を自動判定して分岐するので、参加者はこの複雑さを意識せず同じ操作で扱える。
